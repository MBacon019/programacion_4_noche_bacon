...
fun PantallaPermisos(onTodosConcedidos: () -> Unit = {}) {
    ...
    // 1. Evaluamos dinámicamente el permiso de almacenamiento/fotos según el SDK
    val permisoFotosOAlmacenamiento = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
        android.Manifest.permission.READ_MEDIA_IMAGES
    } else {
        android.Manifest.permission.READ_EXTERNAL_STORAGE
    }

    // 2. Pasamos la variable limpia dentro del remember
    val configuraciones = remember(permisoFotosOAlmacenamiento) {
        listOf(
            ConfigPermiso(
            ...
            ConfigPermiso(
                permisoFotosOAlmacenamiento, // <-- Aquí se usa el permiso correcto según el emulador/dispositivo
                "Fotos",
